...
export default {
  ...
  setup() {
    ...
    function loadImages() {
      server.get('/image/getImageList/' + (++page.current)).then(res => {
    ...
    const settings = reactive({
      ...
      value: 1,
      listStyle: [{value: 0, label: 'Daily Upload', children: [{value: 1, label: 'Show date tag'}, {value: 2, label: 'hide date tag'}]}, {value: 3, label: 'Floder Path'}],
    })
    ...
    return { page, dateList, dailyImgList, imageList, loadImages, deleteByIndex, settings, lookBigImage };
  },
};
</script>
